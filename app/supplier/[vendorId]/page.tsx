...
import { 
  ...
  ShieldCheck, AlertTriangle, Loader2, Eye
} from 'lucide-react';
import { getVendorStatus, submitDocument } from '@/lib/api';
...
function getStepIndex(vendor) {
  ...
  if (total === 0) return 0;
  if (verified === total) return 3;
  if (submitted > 0 || verified > 0) return 2;
  if (pending === total) return 1;
  return 1;
}
...
export default function SupplierPortal() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const res = await getVendorStatus(vendorId);
      ...
      <div className="mb-8 nexus-card p-5">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Vendor ID: {vendorId}</p>
          </div>
          ...
          <div className="nexus-card overflow-hidden">
            ...
            <div className="divide-y divide-slate-100">
              {checklist.map((item, i) => (
                <DocumentRow key={i} item={item} vendorId={vendorId} onUpdate={fetchStatus} />
              ))}
            </div>
          </div>
