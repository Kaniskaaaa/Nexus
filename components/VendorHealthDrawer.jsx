...
import { X, ShieldCheck, ShieldAlert, ShieldX, AlertTriangle, ExternalLink, FileText, CheckCircle, XCircle, Clock, BarChart3 } from 'lucide-react';
...
export default function VendorHealthDrawer({ isOpen, onClose, vendor }) {
  ...
  return (
    <>
      {/* Backdrop */}
      <div 
        className={`fixed inset-0 bg-[#0f1f3d]/20 backdrop-blur-sm z-[60] transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} 
        onClick={onClose}
      />
      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-xl bg-slate-50 shadow-2xl z-[70] transition-transform duration-500 ease-out transform ${isOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <div className={`rounded-xl bg-${health.color}-50 border border-${health.color}-200 p-4 flex items-center gap-4`}>
            <div className={`w-12 h-12 rounded-full bg-${health.color}-100 flex items-center justify-center`}>
              <HealthIcon className={`w-6 h-6 text-${health.color}-600`} />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="nexus-card overflow-hidden">
              <div className={`bg-gradient-to-r ${risk.gradient} p-3 text-white flex items-center gap-3`}>
                <RiskIcon className="w-6 h-6" />
                <div>
                  <p className="text-[10px] uppercase tracking-wider opacity-80">Risk</p>
                  <p className="text-lg font-syne font-bold">{risk.label}</p>
                </div>
              </div>
            </div>
            <div className="nexus-card p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-lg font-syne font-bold text-[#0f1f3d]">{verified}/{total}</p>
              </div>
            </div>
          </div>
          ...
          {checklist.length > 0 && (
            <section className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              <div className="p-4 border-b border-slate-100 flex items-center gap-2 bg-slate-50/50">
                ...
                <h3 className="text-sm font-syne font-bold text-[#0f1f3d]">Compliance Documents</h3>
              </div>
                ...
                {checklist.map((item, i) => (
                  <div key={i} className="px-4 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-[#0f1f3d]">{item.document_name}</p>
          ...
          {vendor.risk_rationale && (
            <section className="bg-white rounded-xl border border-slate-200 p-4">
              <h3 className="text-sm font-syne font-bold text-[#0f1f3d] mb-2 flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-slate-500" />
                Risk Assessment Notes
              </h3>
              <p className="text-sm text-slate-600 leading-relaxed">{vendor.risk_rationale}</p>
            </section>
          )}
          ...
          {auditLog.length > 0 && (
            <section className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                <h3 className="text-sm font-syne font-bold text-[#0f1f3d] flex items-center gap-2">
                  <Clock className="w-4 h-4 text-purple-500" />
                  Recent Agent Activity
                </h3>
                <span className="text-[10px] text-slate-400 font-mono">{auditLog.length} entries</span>
              </div>
              <div className="divide-y divide-slate-100 max-h-[300px] overflow-y-auto">
                {[...auditLog].reverse().slice(0, 5).map((entry, i) => (
                  <div key={i} className="px-4 py-3">
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-[10px] font-bold text-blue-600 uppercase">{entry.agent}</span>
        ...
        <div className="bg-white border-t border-slate-200 p-5 shrink-0">
          <Link 
            ...
            className="w-full nexus-btn-primary py-3 flex items-center justify-center gap-2"
          >
            <ExternalLink className="w-4 h-4" /> Open Supplier Portal
          </Link>
        </div>
      </div>
    </>
